...
async function fetchMovieDetails(movieTitle) {
  try {
    const response = await fetch(
      `/api/movies/booking/title/${encodeURIComponent(movieTitle)}`
    );
...
function groupShowtimesByDomDate(showtimes) {
  ...
  (showtimes || []).forEach((s) => {
    ...
    const key = d
      ? formatShowDateToDomKey(d)
      : formatShowDateToDomKey(s.show_date || s.date || s.show_datetime_iso);
    if (!map[key]) map[key] = [];
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (movie) {
    ...
    if (runtimeElem)
      runtimeElem.textContent = `Movie runtime: ${movie.runtime}`;
...
async function generateDailyShowtimes() {
  ...
  try {
    console.debug(
      "[movie_booking] grouped showtime keys:",
      Object.keys(grouped)
    );
    Object.keys(grouped).forEach((k) =>
      console.debug(`[movie_booking] ${k}: ${grouped[k].length} shows`)
    );
  } catch (e) {
  ...
  if (dateSelector) {
    ...
    displayDates.forEach((dt) => {
      const domKey = `${String(dt.getDate()).padStart(2, "0")} ${dt
      ...
      const domKey = (
        ...
        (item.querySelector(".date") && item.querySelector(".month")
          ? item.querySelector(".date").textContent +
            " " +
            item.querySelector(".month").textContent
          : item.textContent)
      ).trim();
    ...
    const activeKey = active
      ? (
          ...
          (active.querySelector(".date") && active.querySelector(".month")
            ? active.querySelector(".date").textContent +
              " " +
              active.querySelector(".month").textContent
            : active.textContent)
        ).trim()
...
function updateShowtimesForDate() {
  ...
  const dateToShow =
    dateEl && monthEl
      ? dateEl.textContent.trim() + " " + monthEl.textContent.trim()
      : null;
...
function renderShowtimesForDate(dateKey) {
  ...
  const showtimesData = showtimeGrid.dataset.showtimes
    ? JSON.parse(showtimeGrid.dataset.showtimes)
    : [];
  ...
  // Normalize dateKey
  const normalizedDate = targetKey
    ? targetKey.replace(/\s+/g, " ").trim()
    : null;
  ...
  const filtered = normalizedDate
    ...
    : grouped[Object.keys(grouped)[0]] || [];

  if (filtered.length === 0) {
    showtimeGrid.innerHTML =
      '<p class="no-showtimes">No showtimes for this date.</p>';
    return;
  }
...
function initializeShowtimes() {
  ...
  showtimeBtns.forEach((btn) => {
    btn.addEventListener("click", function () {
      ...
      const showtimeId = showtimeIdRaw
        ? Number.parseInt(showtimeIdRaw, 10)
        : Number.NaN; // ensure integer
...
async function showBookingConfirmation(time) {
  const activeDate = document.querySelector(".date-item.active");
  const dateText = activeDate
    ? activeDate.querySelector(".date").textContent
    : "";
  const monthText = activeDate
    ? activeDate.querySelector(".month").textContent
    : "";
  const dayText = activeDate
    ? activeDate.querySelector(".day").textContent
    : "";
...
function setActiveDateByDomKey(domKey) {
  ...
  dateItems.forEach((item) => {
    ...
    if (!dk && !matched) {
      ...
      const composed =
        dateEl && monthEl
          ? (dateEl.textContent + " " + monthEl.textContent).trim()
...
function startAutoRotateDates(intervalMs = 5000) {
  ...
  _autoRotateTimer = setInterval(() => {
    ...
    const currentIndex = dateItems.findIndex((i) =>
