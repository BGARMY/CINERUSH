...
function loadBookingData() {
  ...
  if (bookingData.pricing) {
    const summaryRows = document.querySelectorAll(
      ".payment-summary-bg .summary-row"
    );
    if (summaryRows.length >= 3) {
      summaryRows[0].querySelector(
        "span:last-child"
      ).textContent = `₹${bookingData.pricing.ticketPrice}.00`;
      summaryRows[2].querySelector(
        "span:last-child"
      ).textContent = `₹${bookingData.pricing.total}.00`;
    }
      ...
      confetti.animate(
        [
          ...
          },
          {
            ...
            }px) translateX(${horizontalMovement}px) rotate(720deg)`,
            opacity: 0,
          },
        ],
        {
