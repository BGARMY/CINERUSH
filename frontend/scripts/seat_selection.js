...
async function loadBookedSeats() {
  ...
  const showtimeId =
    bookingInfo.showtimeId || localStorage.getItem("showtimeId");
  if (!showtimeId) return;

  try {
    const response = await fetch(
      `/api/bookings/booked-seats?showtimeId=${showtimeId}`
    );
...
function handleSeatClick(seat) {
  ...
  setTimeout(() => {
    seat.style.transform = seat.classList.contains("selected")
      ? "scale(1.05)"
      : "scale(1)";
  }, 150);
}
...
function updateSeatCount() {
  ...
  if (seatCountDisplay) {
    ...
    seatCountDisplay.textContent =
      seatCount === 1 ? "1 Seat Selected" : `${seatCount} Seats Selected`;
  }
}
...
function suggestBestSeats() {
  ...
  if (availableBestSeats.length >= 2 && selectedSeats.length === 0) {
    setTimeout(() => {
      if (
        confirm("Would you like us to select the best available seats for you?")
      ) {
