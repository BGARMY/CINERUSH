...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function bookMovie(movieTitle) {
    window.location.href = `booking.html?movie=${encodeURIComponent(
      movieTitle
    )}`;
  }
  ...
  bookButtons.forEach((button) => {
    button.addEventListener("click", function () {
      const movieTitle =
        this.parentElement.querySelector(".movie-title").textContent;
      ...
      setTimeout(() => {
        ...
        this.disabled = false;
        window.location.href = `movie_booking.html?movie=${encodeURIComponent(
          movieTitle
        )}`;
      }, 1000);
    });
