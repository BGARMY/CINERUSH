document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (!userId) {
    mainContent.innerHTML =
      '<p class="no-tickets">Please log in to view your booking history.</p>';
  } else {
    try {
      const res = await fetch(
        `/api/bookings/tickets?userId=${encodeURIComponent(userId)}`
      );
      ...
      if (!Array.isArray(bookings) || bookings.length === 0) {
        ...
        bookings.forEach((booking) => {
          const seatCount = booking.seat_number
            ? booking.seat_number.split(",").length
            : 1;
          const bookingDate =
            booking.show_date ||
            (booking.booking_time ? booking.booking_time.split("T")[0] : "");
          const bookingTime =
            booking.show_time ||
            (booking.booking_time
              ? booking.booking_time.split("T")[1].slice(0, 5)
              : "");
      ...
      mainContent.innerHTML =
        '<p class="no-tickets">Error loading booking history. Please try again.</p>';
    }
  }
