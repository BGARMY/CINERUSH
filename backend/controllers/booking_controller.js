...
exports.confirmPayment = async (req, res) => {
  try {
    ...
    await pool.query(
      `UPDATE bookings SET status = ?, amount = ?, transaction_id = ? WHERE id IN (${bookingIds
        .map(() => "?")
        .join(",")})`,
      [
...
exports.getBookedSeatsForShowtime = async (req, res) => {
  try {
    ...
    if (!showtimeId) {
      return res
        .status(400)
        .json({ success: false, message: "Missing showtimeId" });
    }
  ...
  rows.forEach((row) => {
    if (row.seat_number) {
      bookedSeats = bookedSeats.concat(
        row.seat_number.split(",").map((s) => s.trim())
      );
    }
  });
